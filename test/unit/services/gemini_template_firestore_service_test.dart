...
void main() {
  ...
  group('GeminiTemplateFirestoreService', () {
    ...
    setUp(() {
      ...
      when(() => mockFirestore.collection('habit_templates_master')).thenReturn(mockCollectionRef);
    ...
    test('saveGeminiTemplate guarda correctamente el template', () async {
      ...
      verify(() => mockFirestore.collection('habit_templates_master')).called(1);
      verify(() => mockCollectionRef.doc('test_fp')).called(1);
      verify(() => mockDocRef.set(
        any(that: isA<Map<String, dynamic>>().having((m) => m['fingerprint'], 'fingerprint', 'test_fp')),
        any(),
      )).called(1);
    });
  });
