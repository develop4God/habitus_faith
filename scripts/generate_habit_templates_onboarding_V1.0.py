...
def build_prompt(pattern_id: str, scenario: dict, lang_name: str) -> str:
    return f"""
Idioma: {lang_name}
Contexto: {json.dumps(scenario, ensure_ascii=False)}

Genera exactamente 6 hábitos para este perfil. Cada hábito debe ser un objeto JSON con estos campos:
- name (string): acción clara + duración en minutos (ej: "Orar 10 min").
- category (string): una de [spiritual, physical, mental, relational].
- emoji (string): un emoji representativo.
- target_minutes (integer): duración objetivo en minutos.
- difficulty (string): una de [easy, medium, hard].
- subtasks (array opcional): lista de pasos {"title": "..."} (1-3 items).
- predefined_id (string opcional): identificador único de plantilla.
- recommended_time (string opcional): "morning"|"evening"|"any" o "HH:MM".
- notification (object opcional): {"timing": "atEventTime"|"before", "eventTime": "HH:MM"}.

Reglas importantes:
- NO generar hábitos triviales como "respirar" o "beber agua". Si el concepto de respiración aparece, conviértelo en una práctica guiada con propósito.
- Asegura variedad: incluye al menos 3 categorías distintas entre los 6 hábitos.
- Mantén al menos 2 hábitos con target_minutes >= 10.
- Duraciones mínimas: spiritual 5, physical 10, mental 5, relational 5.
- Evita duplicados y sinónimos cercanos.
- Responde SOLO JSON válido con la estructura: {{"pattern_id":"...","habits":[{{...}}...]}}

pattern_id sugerido: {pattern_id}
"""
...
def generate_template(scenario: dict, lang_code: str, lang_name: str, scenario_id: str) -> dict:
    ...
    prompt = build_prompt(pattern_id, scenario, lang_name)
